---
title: "N. Umpqua Temperature Analysis"
author: "J.Hart"
date: "2025-08-23"
output: html_document
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
```


```{r}
# Load packages

library(tidyverse)
library(readxl)
library(janitor)
library(writexl)
library(readr)
```

# 21679180 - Steamboat Above Big Bend Cr.
# 15170014 - Big Bend at mouth
# 21679175 - Steamboat below Big Bend Cr.
# 15170003 - Steamboat above Canton Cr.
# 21679177 - Upper Canton Cr.
# 21433141 - N. Umpqua above Steamboat Cr.
# 15110003 - Fish Cr. at mouth

```{r}
# Read in 2024 temp logger data summaries

daily_water_master <- read_xlsx("2024 Logger Sites/Water/daily_water_master.xlsx")
daily_air_master <- read_xlsx("2024 Logger Sites/Air/daily_air_master.xlsx")
```

# Steamboat Above Big Bend Cr.
```{r S_AboveBigBend}

# Read in data and filter for summer season
S_AboveBigBend <- daily_water_master %>% 
  filter(logger_id == 21679180 & longdate > '2024-06-19' & longdate <= '2024-09-15')

# 2024 Max Temp
S_AboveBigBend_maxt <- S_AboveBigBend %>% 
  summarize(maxt = max(max_temp))
 
# 2024 # Days above 20C
S_AboveBigBend_Days20 <- S_AboveBigBend %>% 
  filter(max_temp > 20) %>% 
  summarize(days20 = n())

-------------------------------------------------------------------------------
### Mean, Min, Max Temp Plot 

# Plot data frame
temp <- S_AboveBigBend

x=as.Date(temp$longdate)
y_lower = 20
y_upper = Inf

# Create temp plot
plot_21679180 <- ggplot() +
  geom_hline(yintercept = 20, linetype = "dotted", size=1) +
  geom_ribbon(aes(x=x,ymin = y_lower, ymax = y_upper), fill = "red1", alpha = 0.10) + 
  geom_line(data=temp, aes(x=as.Date(longdate),y=mean_temp, color="mean_temp"), linewidth =1) +
  geom_line(data=temp, aes(x=as.Date(longdate),y=min_temp, color = "min_temp"), linewidth =0.5) +
  geom_line(data=temp, aes(x=as.Date(longdate), y=max_temp, color= "max_temp"), linewidth =0.5) +
  
  scale_colour_manual("", 
                      breaks = c("mean_temp", "min_temp", "max_temp"),
                      values = c("black", "blue", "red"),
                      labels = c("Mean Temp.", "Min Temp.", "Max Temp.")) +
  scale_x_date(date_breaks = "1 month", date_labels = "%b", expand = c(0,0)) +
  scale_y_continuous(limits = c(9,27),breaks = seq(10,26,by=2)) +
  theme_bw() +
  theme(legend.position = "none") +
  labs(title= "Steamboat above Big Bend Cr.", y="Temperature (°C)", x="") +
  theme(plot.subtitle = element_text(size = 10),
        plot.title = element_text(size=14, face="bold")) +
  theme(axis.text=element_text(size=12),
        axis.title=element_text(size=14))

plot_21679180

# Export plot to file
# ggsave(file="2024 Logger Sites/Water/Fig Plots/plot_21679180.png", height = 3, width = 6)

--------------------------------------------------------------------------------------
## Air/Water Regression Plot

# Prepare air logger data for regression
air <- daily_air_master %>% 
  filter(logger_id == "21679162") %>% # N. Umpqua at Steamboat Air
  mutate(longdate = make_date(year,month,day)) %>% 
  filter(longdate > '2024-06-19' & longdate <= '2024-09-15') %>% 
  mutate(air_mean = mean_temp)  

# Prepare individual water logger data for regression
water <- daily_water_master %>% 
  filter(logger_id == '21679180') %>% 
  mutate(longdate = make_date(year,month,day)) %>%
  filter(longdate > '2024-06-19' & longdate <= '2024-09-15') %>% 
  mutate(water_max = max_temp)

# Join air/water data frames for linear regression
airwater_join <- water %>% 
  left_join(air, by = "longdate")

# Create equation formula for regression equation

lm_eqn <- function(df){
    m <- lm(water_max ~ air_mean, df);
    eq <- substitute(italic(y) == a + b %.% italic(x)*","~~italic(r)^2~"="~r2, 
         list(a = format(unname(coef(m)[1]), digits = 3),
              b = format(unname(coef(m)[2]), digits = 3),
             r2 = format(summary(m)$adj.r.squared, digits = 3)))
    as.character(as.expression(eq));
}                 

# Calculate Pearson correlation coefficient
correlation <- cor(water$max_temp, air$mean_temp, method = 'pearson')
correlation

fit <- lm(airwater_join$water_max ~ airwater_join$air_mean)
summary(fit)


# Create regression plot
reg_plot <- ggplot(airwater_join, aes(x=air_mean, y = water_max)) +
  geom_point(color='blue') +
  geom_smooth(method = "lm", se = TRUE, colour = 'black') +
  theme_light() + 
 labs(title="Steamboat above Big Bend Cr. (Air - N. Umpqua at Steamboat)",
       subtitle = "Max Water Temp ~ Mean Air Temp",
       x="Daily Mean Air temperature (°C)",
       y="Daily Max Water temperature (°C)") + 
geom_text(x = 16, y = 24, label = lm_eqn(airwater_join), parse = TRUE)  + 
geom_text(x = 16, y = 23, label = paste("Pearson R:", round(correlation,3))) +
  theme(plot.title = element_text(size=12, face="bold"),
        plot.subtitle = element_text(size = 11)) +
  theme(axis.text=element_text(size=11),
        axis.title=element_text(size=14))

reg_plot

# Export plot to file
#ggsave(file="2024 Logger Sites/Water/Fig Plots/regression_21679180.png", height = 4, width = 6)

```

# Big Bend Cr. at mouth
```{r Big Bend}
# Read in Big Bend USB Data

list_of_files <- list.files(path = "USB Files/Big Bend",
                            recursive = TRUE,
                            pattern = "\\.csv$",
                            full.names = TRUE)

BigBend <- readr::read_csv(list_of_files, id = "file_name") 

# Clean and format data for analysis
BigBend <- BigBend %>% 
  clean_names() %>% 
  mutate(site = "Big Bend Cr. at mouth") %>% 
  mutate(temp_c = (temp - 32) * (5/9)) %>% 
  mutate(logger_id = "15170014") %>% 
  mutate(longdate = as.Date(date,format = "%m/%d/%Y")) %>% 
  separate(longdate,
                  into = c('year', 'month', 'day'),
                  sep= '-',
                  remove = FALSE) %>% 
  mutate(month_day = format(longdate, "%m-%d"))
  
# Calculate daily summary stats
BigBend <- BigBend %>% 
  group_by(longdate, month_day,year,logger_id) %>% 
  summarise(mean_temp = round(mean(temp_c), digits=2),
            min_temp = round(min(temp_c), digits=2),
            max_temp = round(max(temp_c), digits=2))

## Calculate temperature statistics

# Max T (2024)
BigBend_maxt_2024 <- BigBend %>% 
  filter(longdate > '2024-06-19' & longdate <= '2024-09-15') %>% 
  group_by(logger_id) %>% 
  summarise(maxt = max(max_temp))

# Days >20C (2024)
BigBend_Days20_2024 <- BigBend %>% 
  filter(longdate > '2024-06-19' & longdate <= '2024-09-15') %>% 
  filter(max_temp > 20) %>% 
  group_by(logger_id) %>% 
  summarise(days_over20 = n())

# Max T (10-yr Avg.)
BigBend_maxt_10yrAvg <- BigBend %>% 
  filter(month_day > '06-19' & month_day <= '09-15') %>% 
  group_by(logger_id,year) %>% 
  summarise(maxt = max(max_temp)) %>% 
  summarise(maxt_avg = mean(maxt))

# Days >20C (10-yr Avg.)
BigBend_Days20_10yrAvg <- BigBend %>% 
  filter(month_day > '06-19' & month_day <= '09-15') %>% 
  filter(max_temp > 20) %>% 
  group_by(logger_id,year) %>%
  summarise(days_over20 = n()) %>% 
  summarise(days_over20_avg = sum(days_over20) / 10)

# Max T (Drought)  
  BigBend_maxt_drought <- BigBend %>% 
  filter(year == '2021' | year == '2022') %>% 
  filter(month_day > '06-19' & month_day <= '09-15') %>% 
  group_by(logger_id) %>% 
  summarise(maxt = max(max_temp))
  
# Days >20C (Drought)
  BigBend_Days20_drought <- BigBend %>% 
  filter(year == '2021' | year == '2022') %>% 
  filter(month_day > '06-19' & month_day <= '09-15') %>% 
  filter(max_temp > 20) %>% 
  group_by(logger_id,year) %>% 
  summarise(days_over20 = n()) %>% 
  summarise(days_over20_avg = sum(days_over20) / 2)
  
-------------------------------------------------------------------------------
### Mean, Min, Max Temp Plot 

# Data frame for plot
temp <- BigBend %>% 
  filter(longdate > '2024-06-19' & longdate <= '2024-09-15')
  
x=as.Date(temp$longdate)
y_lower = 20
y_upper = Inf

# Create temp plot
plot_15170014 <- ggplot() +
  geom_hline(yintercept = 20, linetype = "dotted", size=1) +
  geom_ribbon(aes(x=x,ymin = y_lower, ymax = y_upper), fill = "red1", alpha = 0.10) + 
  geom_line(data=temp, aes(x=as.Date(longdate),y=mean_temp, color="mean_temp"), linewidth =1) +
  geom_line(data=temp, aes(x=as.Date(longdate),y=min_temp, color = "min_temp"), linewidth =0.5) +
  geom_line(data=temp, aes(x=as.Date(longdate), y=max_temp, color= "max_temp"), linewidth =0.5) +
  
  scale_colour_manual("", 
                      breaks = c("mean_temp", "min_temp", "max_temp"),
                      values = c("black", "blue", "red"),
                      labels = c("Mean Temp.", "Min Temp.", "Max Temp.")) +
  scale_x_date(date_breaks = "1 month", date_labels = "%b", expand = c(0,0)) +
  scale_y_continuous(limits = c(9,27),breaks = seq(10,26,by=2)) +
  theme_bw() +
  theme(legend.position = "none") +
  labs(title= "Big Bend Cr. at mouth", y="Temperature (°C)", x="") +
  theme(plot.subtitle = element_text(size = 10),
        plot.title = element_text(size=14, face="bold")) +
  theme(axis.text=element_text(size=12),
        axis.title=element_text(size=14))

plot_15170014

# Export plot to file 
#ggsave(file="2024 Logger Sites/Water/Fig Plots/plot_15170014.png", height = 3, width = 6)

--------------------------------------------------------------------------------------
## Air/Water Regression Plot

# Prepare air logger data for regression
air <- daily_air_master %>% 
  filter(logger_id == "21679162") %>% # N. Umpqua at Steamboat Air
  mutate(longdate = make_date(year,month,day)) %>% 
  filter(longdate > '2024-06-19' & longdate <= '2024-09-15') %>% 
  mutate(air_mean = mean_temp)  

# Prepare individual water logger data for regression
water <- BigBend %>% 
  filter(longdate > '2024-06-19' & longdate <= '2024-09-15') %>% 
  mutate(water_max = max_temp)

# Join air/water data frames for linear regression
airwater_join <- water %>% 
  left_join(air, by = "longdate")

# Create equation formula for regression equation

lm_eqn <- function(df){
    m <- lm(water_max ~ air_mean, df);
    eq <- substitute(italic(y) == a + b %.% italic(x)*","~~italic(r)^2~"="~r2, 
         list(a = format(unname(coef(m)[1]), digits = 3),
              b = format(unname(coef(m)[2]), digits = 3),
             r2 = format(summary(m)$adj.r.squared, digits = 3)))
    as.character(as.expression(eq));
}                 

# Calculate Pearson correlation coefficient
correlation <- cor(water$max_temp, air$mean_temp, method = 'pearson')
correlation

fit <- lm(airwater_join$water_max ~ airwater_join$air_mean)
summary(fit)

  
# Create regression plot
reg_plot <- ggplot(airwater_join, aes(x=air_mean, y = water_max)) +
  geom_point(color='blue') +
  geom_smooth(method = "lm", se = TRUE, colour = 'black') +
  theme_light() + 
 labs(title="Big Bend Cr. at mouth (Air - N. Umpqua at Steamboat)",
       subtitle = "Max Water Temp ~ Mean Air Temp",
       x="Daily Mean Air temperature (°C)",
       y="Daily Max Water temperature (°C)") + 
geom_text(x = 16, y = 19, label = lm_eqn(airwater_join), parse = TRUE)  + 
geom_text(x = 16, y = 18, label = paste("Pearson R:", round(correlation,3))) +
  theme(plot.title = element_text(size=12, face="bold"),
        plot.subtitle = element_text(size = 11)) +
  theme(axis.text=element_text(size=11),
        axis.title=element_text(size=14))

reg_plot

# Export plot to file
#ggsave(file="2024 Logger Sites/Water/Fig Plots/regression_15170014.png", height = 4, width = 6)
  
```

# Steamboat Below Big Bend Cr.
```{r S_BelowBigBend}

# Read in data and filter for summer season
S_BelowBigBend <- daily_water_master %>% 
  filter(logger_id == 21679175 & longdate > '2024-06-19' & longdate <= '2024-09-15')

# 2024 Max Temp
S_BelowBigBend_maxt <- S_BelowBigBend %>% 
  summarize(maxt = max(max_temp))

# 2024 # Days above 20C
S_BelowBigBend_Days20 <- S_BelowBigBend %>% 
  filter(max_temp > 20) %>% 
  summarize(days20 = n())

-------------------------------------------------------------------------------
### Mean, Min, Max Temp Plot 

# Plot data frame
temp <- S_BelowBigBend

x=as.Date(temp$longdate)
y_lower = 20
y_upper = Inf

# Create temp plot
plot_21679175 <- ggplot() +
  geom_hline(yintercept = 20, linetype = "dotted", size=1) +
  geom_ribbon(aes(x=x,ymin = y_lower, ymax = y_upper), fill = "red1", alpha = 0.10) + 
  geom_line(data=temp, aes(x=as.Date(longdate),y=mean_temp, color="mean_temp"), linewidth =1) +
  geom_line(data=temp, aes(x=as.Date(longdate),y=min_temp, color = "min_temp"), linewidth =0.5) +
  geom_line(data=temp, aes(x=as.Date(longdate), y=max_temp, color= "max_temp"), linewidth =0.5) +
  
  scale_colour_manual("", 
                      breaks = c("mean_temp", "min_temp", "max_temp"),
                      values = c("black", "blue", "red"),
                      labels = c("Mean Temp.", "Min Temp.", "Max Temp.")) +
  scale_x_date(date_breaks = "1 month", date_labels = "%b", expand = c(0,0)) +
  scale_y_continuous(limits = c(9,27),breaks = seq(10,26,by=2)) +
  theme_bw() +
  theme(legend.position = "none") +
  labs(title= "Steamboat below Big Bend Cr.", y="Temperature (°C)", x="") +
  theme(plot.subtitle = element_text(size = 10),
        plot.title = element_text(size=14, face="bold")) +
  theme(axis.text=element_text(size=12),
        axis.title=element_text(size=14))

plot_21679175

# Export plot to file 
#ggsave(file="2024 Logger Sites/Water/Fig Plots/plot_21679175.png", height = 3, width = 6)

--------------------------------------------------------------------------------------
## Air/Water Regression Plot

# Prepare air logger data for regression
air <- daily_air_master %>% 
  filter(logger_id == "21679162") %>% # N. Umpqua at Steamboat Air
  mutate(longdate = make_date(year,month,day)) %>% 
  filter(longdate > '2024-06-19' & longdate <= '2024-09-15') %>% 
  mutate(air_mean = mean_temp)  

# Prepare individual water logger data for regression
water <- daily_water_master %>% 
  filter(logger_id == '21679175') %>% 
  mutate(longdate = make_date(year,month,day)) %>%
  filter(longdate > '2024-06-19' & longdate <= '2024-09-15') %>% 
  mutate(water_max = max_temp)

# Join air/water data frames for linear regression
airwater_join <- water %>% 
  left_join(air, by = "longdate")

# Create equation formula for regression equation

lm_eqn <- function(df){
    m <- lm(water_max ~ air_mean, df);
    eq <- substitute(italic(y) == a + b %.% italic(x)*","~~italic(r)^2~"="~r2, 
         list(a = format(unname(coef(m)[1]), digits = 3),
              b = format(unname(coef(m)[2]), digits = 3),
             r2 = format(summary(m)$adj.r.squared, digits = 3)))
    as.character(as.expression(eq));
}                 

# Calculate Pearson correlation coefficient
correlation <- cor(water$max_temp, air$mean_temp, method = 'pearson')
correlation

fit <- lm(airwater_join$water_max ~ airwater_join$air_mean)
summary(fit)


# Create regression plot

reg_plot <- ggplot(airwater_join, aes(x=air_mean, y = water_max)) +
  geom_point(color='blue') +
  geom_smooth(method = "lm", se = TRUE, colour = 'black') +
  theme_light() + 
 labs(title="Steamboat below Big Bend Cr. (Air - N. Umpqua at Steamboat)",
       subtitle = "Max Water Temp ~ Mean Air Temp",
       x="Daily Mean Air temperature (°C)",
       y="Daily Max Water temperature (°C)") + 
geom_text(x = 16, y = 21, label = lm_eqn(airwater_join), parse = TRUE)  + 
geom_text(x = 16, y = 20, label = paste("Pearson R:", round(correlation,3))) +
  theme(plot.title = element_text(size=12, face="bold"),
        plot.subtitle = element_text(size = 11)) +
  theme(axis.text=element_text(size=11),
        axis.title=element_text(size=14))

reg_plot

# Export plot to file
#ggsave(file="2024 Logger Sites/Water/Fig Plots/regression_21679175.png", height = 4, width = 6)

```

# Steamboat Above Canton Cr.
```{r Steamboat Above Canton}

# Read in Steamboat Above Canton USB Data
list_of_files <- list.files(path = "USB Files/SteamAboveCanton",
                            recursive = TRUE,
                            pattern = "\\.csv$",
                            full.names = TRUE)

SteamboatCant <- readr::read_csv(list_of_files, id = "file_name") 

# Clean and format data for analysis
SteamboatCant <- SteamboatCant %>% 
  clean_names() %>% 
  mutate(site = "Steamboat above Canton Cr.") %>% 
  mutate(temp_c = (temp - 32) * (5/9)) %>% 
  mutate(logger_id = "15170003") %>% 
  mutate(longdate = as.Date(date,format = "%m/%d/%Y")) %>% 
  separate(longdate,
                  into = c('year', 'month', 'day'),
                  sep= '-',
                  remove = FALSE) %>% 
  mutate(month_day = format(longdate, "%m-%d"))
  
  
# Calculate daily summary stats
SteamboatCant <- SteamboatCant %>% 
  group_by(longdate, month_day, year, logger_id) %>% 
  summarise(mean_temp = round(mean(temp_c), digits=2),
            min_temp = round(min(temp_c), digits=2),
            max_temp = round(max(temp_c), digits=2))

# Calculate temperature statistics

# Max T (2024)
SteamboatCant_maxt_2024 <- SteamboatCant %>% 
  filter(longdate > '2024-06-19' & longdate <= '2024-09-15') %>% 
  group_by(logger_id) %>% 
  summarise(maxt = max(max_temp))

# Days >20C (2024)
SteamboatCant_Days20_2024 <- SteamboatCant %>% 
  filter(longdate > '2024-06-19' & longdate <= '2024-09-15') %>% 
  filter(max_temp > 20) %>% 
  group_by(logger_id) %>% 
  summarise(days_over20 = n())

# Max T (10-yr Avg.)
SteamboatCant_maxt_10yrAvg <- SteamboatCant %>% 
  filter(month_day > '06-19' & month_day <= '09-15') %>% 
  group_by(logger_id,year) %>% 
  summarise(maxt = max(max_temp)) %>% 
  summarise(maxt_avg = mean(maxt))

# Days >20C (10-yr Avg.)
SteamboatCant_Days20_10yrAvg <- SteamboatCant %>% 
  filter(month_day > '06-19' & month_day <= '09-15') %>% 
  filter(max_temp > 20) %>% 
  group_by(logger_id,year) %>%
  summarise(days_over20 = n()) %>% 
  summarise(days_over20_avg = sum(days_over20) / 10)

# Max T (Drought)  
  SteamboatCant_maxt_drought <- SteamboatCant %>% 
  filter(year == '2021' | year == '2022') %>% 
  filter(month_day > '06-19' & month_day <= '09-15') %>% 
  group_by(logger_id) %>% 
  summarise(maxt = max(max_temp))
  
# Days >20C (Drought)
 SteamboatCant_Days20_drought <- SteamboatCant %>% 
  filter(year == '2021' | year == '2022') %>% 
  filter(month_day > '06-19' & month_day <= '09-15') %>% 
  filter(max_temp > 20) %>% 
  group_by(logger_id,year) %>% 
  summarise(days_over20 = n()) %>% 
  summarise(days_over20_avg = sum(days_over20) / 2)
 
-------------------------------------------------------------------------------
### Mean, Min, Max Temp Plot 

# Data frame for plot
temp <- SteamboatCant %>% 
  filter(longdate > '2024-06-19' & longdate <= '2024-09-15')

x=as.Date(temp$longdate)
y_lower = 20
y_upper = Inf

# Create temp plot
plot_15170003 <- ggplot() +
  geom_hline(yintercept = 20, linetype = "dotted", size=1) +
  geom_ribbon(aes(x=x,ymin = y_lower, ymax = y_upper), fill = "red1", alpha = 0.10) + 
  geom_line(data=temp, aes(x=longdate,y=mean_temp, color="mean_temp"), linewidth =1) +
  geom_line(data=temp, aes(x=longdate,y=min_temp, color = "min_temp"), linewidth =0.5) +
  geom_line(data=temp, aes(x=longdate, y=max_temp, color= "max_temp"), linewidth =0.5) +
  
  scale_colour_manual("", 
                      breaks = c("mean_temp", "min_temp", "max_temp"),
                      values = c("black", "blue", "red"),
                      labels = c("Mean Temp.", "Min Temp.", "Max Temp.")) +
  scale_x_date(date_breaks = "1 month", date_labels = "%b", expand = c(0,0)) +
  scale_y_continuous(limits = c(9,27),breaks = seq(10,26,by=2)) +
  theme_bw() +
  theme(legend.position = "none") +
  labs(title= "Steamboat above Canton Cr.", y="Temperature (°C)", x="") +
  theme(plot.subtitle = element_text(size = 10),
        plot.title = element_text(size=14, face="bold")) +
  theme(axis.text=element_text(size=12),
        axis.title=element_text(size=14))


plot_15170003

# Export plot to file 
#ggsave(file="2024 Logger Sites/Water/Fig Plots/plot_15170003.png", height = 3, width = 6)

--------------------------------------------------------------------------------------
## Air/Water Regression Plot

# Prepare air logger data for regression
air <- daily_air_master %>% 
  filter(logger_id == "21679162") %>% # N. Umpqua at Steamboat Air
  mutate(longdate = make_date(year,month,day)) %>% 
  filter(longdate > '2024-06-19' & longdate <= '2024-09-15') %>% 
  mutate(air_mean = mean_temp)  

# Prepare individual water logger data for regression
water <- SteamboatCant  %>% 
  filter(longdate > '2024-06-19' & longdate <= '2024-09-15') %>% 
  mutate(water_max = max_temp)

# Join air/water data frames for linear regression
airwater_join <- water %>% 
  left_join(air, by = "longdate")

# Create equation formula for regression equation

lm_eqn <- function(df){
    m <- lm(water_max ~ air_mean, df);
    eq <- substitute(italic(y) == a + b %.% italic(x)*","~~italic(r)^2~"="~r2, 
         list(a = format(unname(coef(m)[1]), digits = 3),
              b = format(unname(coef(m)[2]), digits = 3),
             r2 = format(summary(m)$adj.r.squared, digits = 3)))
    as.character(as.expression(eq));
}                 

# Calculate Pearson correlation coefficient
correlation <- cor(water$max_temp, air$mean_temp, method = 'pearson')
correlation

fit <- lm(airwater_join$water_max ~ airwater_join$air_mean)
summary(fit)

  
# Create regression plot
reg_plot <- ggplot(airwater_join, aes(x=air_mean, y = water_max)) +
  geom_point(color='blue') +
  geom_smooth(method = "lm", se = TRUE, colour = 'black') +
  theme_light() + 
 labs(title="Steamboat above Canton Cr. (Air - N. Umpqua at Steamboat)",
       subtitle = "Max Water Temp ~ Mean Air Temp",
       x="Daily Mean Air temperature (°C)",
       y="Daily Max Water temperature (°C)") + 
geom_text(x = 16, y = 24, label = lm_eqn(airwater_join), parse = TRUE)  + 
geom_text(x = 16, y = 23, label = paste("Pearson R:", round(correlation,3))) +
  theme(plot.title = element_text(size=12, face="bold"),
        plot.subtitle = element_text(size = 11)) +
  theme(axis.text=element_text(size=11),
        axis.title=element_text(size=14))

reg_plot

# Export plot to file
#ggsave(file="2024 Logger Sites/Water/Fig Plots/regression_15170003.png", height = 4, width = 6)
```

# Upper Canton Cr.
```{r UpperCanton}

# Read in data and filter for summer season
UpperCanton <- daily_water_master %>% 
  filter(logger_id == 21679177 & longdate > '2024-06-19' & longdate <= '2024-09-15')

# 2024 Max Temp
UpperCanton_maxt <- UpperCanton %>% 
  summarize(maxt = max(max_temp))

# 2024 # Days above 20C
UpperCanton_Days20 <- UpperCanton %>% 
  filter(max_temp > 20) %>% 
  summarize(days20 = n())

-------------------------------------------------------------------------------
### Mean, Min, Max Temp Plot 

# Plot data frame
temp <- UpperCanton

x=as.Date(temp$longdate)
y_lower = 20
y_upper = Inf

# Create temp plot
plot_21679177 <- ggplot() +
  geom_hline(yintercept = 20, linetype = "dotted", size=1) +
  geom_ribbon(aes(x=x,ymin = y_lower, ymax = y_upper), fill = "red1", alpha = 0.10) + 
  geom_line(data=temp, aes(x=as.Date(longdate),y=mean_temp, color="mean_temp"), linewidth =1) +
  geom_line(data=temp, aes(x=as.Date(longdate),y=min_temp, color = "min_temp"), linewidth =0.5) +
  geom_line(data=temp, aes(x=as.Date(longdate), y=max_temp, color= "max_temp"), linewidth =0.5) +
  
  scale_colour_manual("", 
                      breaks = c("mean_temp", "min_temp", "max_temp"),
                      values = c("black", "blue", "red"),
                      labels = c("Mean Temp.", "Min Temp.", "Max Temp.")) +
  scale_x_date(date_breaks = "1 month", date_labels = "%b", expand = c(0,0)) +
  scale_y_continuous(limits = c(9,27),breaks = seq(10,26,by=2)) +
  theme_bw() +
  theme(legend.position = "none") +
  labs(title= "Upper Canton Cr.", y="Temperature (°C)", x="") +
  theme(plot.subtitle = element_text(size = 10),
        plot.title = element_text(size=14, face="bold")) +
  theme(axis.text=element_text(size=12),
        axis.title=element_text(size=14))


plot_21679177

# Export plot to file 
#ggsave(file="2024 Logger Sites/Water/Fig Plots/plot_21679177.png", height = 3, width = 6)

--------------------------------------------------------------------------------------
## Air/Water Regression Plot

# Prepare air logger data for regression
air <- daily_air_master %>% 
  filter(logger_id == "21679162") %>% # N. Umpqua at Steamboat Air
  mutate(longdate = make_date(year,month,day)) %>% 
  filter(longdate > '2024-06-19' & longdate <= '2024-09-15') %>% 
  mutate(air_mean = mean_temp)  

# Prepare individual water logger data for regression
water <- daily_water_master %>% 
  filter(logger_id == '21679177') %>% 
  mutate(longdate = make_date(year,month,day)) %>%
  filter(longdate > '2024-06-19' & longdate <= '2024-09-15') %>% 
  mutate(water_max = max_temp)

# Join air/water data frames for linear regression
airwater_join <- water %>% 
  left_join(air, by = "longdate")

# Create equation formula for regression equation

lm_eqn <- function(df){
    m <- lm(water_max ~ air_mean, df);
    eq <- substitute(italic(y) == a + b %.% italic(x)*","~~italic(r)^2~"="~r2, 
         list(a = format(unname(coef(m)[1]), digits = 3),
              b = format(unname(coef(m)[2]), digits = 3),
             r2 = format(summary(m)$adj.r.squared, digits = 3)))
    as.character(as.expression(eq));
}                 

# Calculate Pearson correlation coefficient
correlation <- cor(water$max_temp, air$mean_temp, method = 'pearson')
correlation

fit <- lm(airwater_join$water_max ~ airwater_join$air_mean)
summary(fit)


# Create regression plot
reg_plot <- ggplot(airwater_join, aes(x=air_mean, y = water_max)) +
  geom_point(color='blue') +
  geom_smooth(method = "lm", se = TRUE, colour = 'black') +
  theme_light() + 
 labs(title="Upper Canton Cr. (Air - N. Umpqua at Steamboat)",
       subtitle = "Max Water Temp ~ Mean Air Temp",
       x="Daily Mean Air temperature (°C)",
       y="Daily Max Water temperature (°C)") + 
geom_text(x = 16, y = 21, label = lm_eqn(airwater_join), parse = TRUE)  + 
geom_text(x = 16, y = 20, label = paste("Pearson R:", round(correlation,3))) +
  theme(plot.title = element_text(size=12, face="bold"),
        plot.subtitle = element_text(size = 11)) +
  theme(axis.text=element_text(size=11),
        axis.title=element_text(size=14))

reg_plot

# Export plot to file
#ggsave(file="2024 Logger Sites/Water/Fig Plots/regression_21679177.png", height = 4, width = 6)

```

# N. Umpqua above Steamboat
```{r NUmpqua_AboveSteam}

# Read in data and filter for summer season
NUmpqua_AboveSteam <- daily_water_master %>% 
  filter(logger_id == 21433141 & longdate > '2024-06-19' & longdate <= '2024-09-15')

# 2024 Max Temp
NUmpqua_AboveSteam_maxt <- NUmpqua_AboveSteam %>% 
  summarize(maxt = max(max_temp))

# 2024 # Days above 20C
NUmpqua_AboveSteam_Days20 <- NUmpqua_AboveSteam %>% 
  filter(max_temp > 20) %>% 
  summarize(days20 = n())

-------------------------------------------------------------------------------
### Mean, Min, Max Temp Plot 

# Plot data frame
temp <- NUmpqua_AboveSteam

x=as.Date(temp$longdate)
y_lower = 20
y_upper = Inf

# Create temp plot
plot_21433141 <- ggplot() +
  geom_hline(yintercept = 20, linetype = "dotted", size=1) +
  geom_ribbon(aes(x=x,ymin = y_lower, ymax = y_upper), fill = "red1", alpha = 0.10) + 
  geom_line(data=temp, aes(x=as.Date(longdate),y=mean_temp, color="mean_temp"), linewidth =1) +
  geom_line(data=temp, aes(x=as.Date(longdate),y=min_temp, color = "min_temp"), linewidth =0.5) +
  geom_line(data=temp, aes(x=as.Date(longdate), y=max_temp, color= "max_temp"), linewidth =0.5) +
  
  scale_colour_manual("", 
                      breaks = c("mean_temp", "min_temp", "max_temp"),
                      values = c("black", "blue", "red"),
                      labels = c("Mean Temp.", "Min Temp.", "Max Temp.")) +
  scale_x_date(date_breaks = "1 month", date_labels = "%b", expand = c(0,0)) +
  scale_y_continuous(limits = c(9,27),breaks = seq(10,26,by=2)) +
  theme_bw() +
  theme(legend.position = "none") +
  labs(title= "N. Umpqua above Steamboat Cr.", y="Temperature (°C)", x="") +
  theme(plot.subtitle = element_text(size = 10),
        plot.title = element_text(size=14, face="bold")) +
  theme(axis.text=element_text(size=12),
        axis.title=element_text(size=14))


plot_21433141

# Export plot to file 
#ggsave(file="2024 Logger Sites/Water/Fig Plots/plot_21433141.png", height = 3, width = 6)

--------------------------------------------------------------------------------------
## Air/Water Regression Plot

# Prepare air logger data for regression
air <- daily_air_master %>% 
  filter(logger_id == "21679162") %>% # N. Umpqua at Steamboat Air
  mutate(longdate = make_date(year,month,day)) %>% 
  filter(longdate > '2024-06-19' & longdate <= '2024-09-15') %>% 
  mutate(air_mean = mean_temp)  

# Prepare individual water logger data for regression
water <- daily_water_master %>% 
  filter(logger_id == '21433141') %>% 
  mutate(longdate = make_date(year,month,day)) %>%
  filter(longdate > '2024-06-19' & longdate <= '2024-09-15') %>% 
  mutate(water_max = max_temp)

# Join air/water data frames for linear regression
airwater_join <- water %>% 
  left_join(air, by = "longdate")

# Create equation formula for regression equation

lm_eqn <- function(df){
    m <- lm(water_max ~ air_mean, df);
    eq <- substitute(italic(y) == a + b %.% italic(x)*","~~italic(r)^2~"="~r2, 
         list(a = format(unname(coef(m)[1]), digits = 3),
              b = format(unname(coef(m)[2]), digits = 3),
             r2 = format(summary(m)$adj.r.squared, digits = 3)))
    as.character(as.expression(eq));
}                 

# Calculate Pearson correlation coefficient
correlation <- cor(water$max_temp, air$mean_temp, method = 'pearson')
correlation

fit <- lm(airwater_join$water_max ~ airwater_join$air_mean)
summary(fit)


# Create regression plot
reg_plot <- ggplot(airwater_join, aes(x=air_mean, y = water_max)) +
  geom_point(color='blue') +
  geom_smooth(method = "lm", se = TRUE, colour = 'black') +
  theme_light() + 
 labs(title="North Umpqua above Steamboat Cr. (Air - N. Umpqua at Steamboat)",
       subtitle = "Max Water Temp ~ Mean Air Temp",
       x="Daily Mean Air temperature (°C)",
       y="Daily Max Water temperature (°C)") + 
geom_text(x = 15, y = 18, label = lm_eqn(airwater_join), parse = TRUE)  + 
geom_text(x = 15, y = 17, label = paste("Pearson R:", round(correlation,3))) +
  theme(plot.title = element_text(size=12, face="bold"),
        plot.subtitle = element_text(size = 11)) +
  theme(axis.text=element_text(size=11),
        axis.title=element_text(size=14))

reg_plot

#Export plot to file
#ggsave(file="2024 Logger Sites/Water/Fig Plots/regression_21433141.png", height = 4, width = 6)
```

# Fish Cr. at mouth
```{r Fish Cr}
# Read in Fish Cr. USB Data

list_of_files <- list.files(path = "USB Files/Fish",
                            recursive = TRUE,
                            pattern = "\\.csv$",
                            full.names = TRUE)

FishCr <- readr::read_csv(list_of_files, id = "file_name") 

# Clean and format data for analysis
FishCr<- FishCr %>% 
  clean_names() %>% 
  mutate(site = "Fish Cr. at mouth") %>% 
  mutate(temp_c = (temp - 32) * (5/9)) %>% 
  mutate(logger_id = "15110003") %>% 
  mutate(longdate = as.Date(date,format = "%m/%d/%Y")) %>% 
  separate(longdate,
                  into = c('year', 'month', 'day'),
                  sep= '-',
                  remove = FALSE) %>% 
  mutate(month_day = format(longdate, "%m-%d"))
  
  
# Calculate daily summary stats
FishCr <- FishCr %>% 
  group_by(longdate,month_day,year,logger_id) %>% 
  summarise(mean_temp = round(mean(temp_c), digits=2),
            min_temp = round(min(temp_c), digits=2),
            max_temp = round(max(temp_c), digits=2))

# Calculate temperature statistics

# Max T (2024)
FishCr_maxt_2024 <-FishCr %>% 
  filter(longdate > '2024-06-19' & longdate <= '2024-09-15') %>% 
  group_by(logger_id) %>% 
  summarise(maxt = max(max_temp))

# Days >20C (2024)
FishCr_Days20_2024 <- FishCr %>% 
  filter(longdate > '2024-06-19' & longdate <= '2024-09-15') %>% 
  filter(max_temp > 20) %>% 
  group_by(logger_id) %>% 
  summarise(days_over20 = n())

# Max T (10-yr Avg.)
FishCr_maxt_10yrAvg <- FishCr %>% 
  filter(month_day > '06-19' & month_day <= '09-15') %>% 
  group_by(logger_id,year) %>% 
  summarise(maxt = max(max_temp)) %>% 
  summarise(maxt_avg = mean(maxt))

# Days >20C (10-yr Avg.)
FishCr_Days20_10yrAvg <- FishCr %>% 
  filter(month_day > '06-19' & month_day <= '09-15') %>% 
  filter(max_temp > 20) %>% 
  group_by(year) %>% 
  summarise(days_over20 = n()) %>% 
  summarise(days_over20_avg = sum(days_over20) / 10)

# Max T (Drought)  
  FishCr_maxt_drought <- FishCr %>% 
  filter(year == '2021' | year == '2022') %>% 
  filter(month_day > '06-19' & month_day <= '09-15') %>% 
  group_by(logger_id) %>% 
  summarise(maxt = max(max_temp))
  
# Days >20C (Drought)
 FishCr_Days20_drought <- FishCr %>% 
  filter(year == '2021' | year == '2022') %>% 
  filter(month_day > '06-19' & month_day <= '09-15') %>% 
  filter(max_temp > 20) %>% 
  group_by(logger_id,year) %>% 
  summarise(days_over20 = n()) %>% 
  summarise(days_over20_avg = sum(days_over20) / 2)
 
-------------------------------------------------------------------------------
### Mean, Min, Max Temp Plot 

# Data frame for plot
 temp <- FishCr %>% 
  filter(longdate > '2024-06-19' & longdate <= '2024-09-15')

x=as.Date(temp$longdate)
y_lower = 20
y_upper = Inf

# Create temp plot
plot_15110003 <- ggplot() +
  geom_hline(yintercept = 20, linetype = "dotted", size=1) +
  geom_ribbon(aes(x=x,ymin = y_lower, ymax = y_upper), fill = "red1", alpha = 0.10) + 
  geom_line(data=temp, aes(x=longdate,y=mean_temp, color="mean_temp"), linewidth =1) +
  geom_line(data=temp, aes(x=longdate,y=min_temp, color = "min_temp"), linewidth =0.5) +
  geom_line(data=temp, aes(x=longdate, y=max_temp, color= "max_temp"), linewidth =0.5) +
  
  scale_colour_manual("", 
                      breaks = c("mean_temp", "min_temp", "max_temp"),
                      values = c("black", "blue", "red"),
                      labels = c("Mean Temp.", "Min Temp.", "Max Temp.")) +
  scale_x_date(date_breaks = "1 month", date_labels = "%b", expand = c(0,0)) +
  scale_y_continuous(limits = c(9,27),breaks = seq(10,26,by=2)) +
  theme_bw() +
  theme(legend.position = "none") +
  labs(title= "Fish Cr. at mouth", y="Temperature (°C)", x="") +
  theme(plot.subtitle = element_text(size = 10),
        plot.title = element_text(size=14, face="bold")) +
  theme(axis.text=element_text(size=12),
        axis.title=element_text(size=14))


plot_15110003

# Export plot to file 
#ggsave(file="2024 Logger Sites/Water/Fig Plots/plot_15110003.png", height = 3, width = 6)

--------------------------------------------------------------------------------------
## Air/Water Regression Plot

# Prepare air logger data for regression
air <- daily_air_master %>% 
  filter(logger_id == "21433138") %>% # Copeland Cr. at mouth
  mutate(longdate = make_date(year,month,day)) %>% 
  filter(longdate > '2024-06-19' & longdate <= '2024-09-15') %>% 
  mutate(air_mean = mean_temp)  

# Prepare individual water logger data for regression
water <- FishCr %>% 
  filter(longdate > '2024-06-19' & longdate <= '2024-09-15') %>% 
  mutate(water_max = max_temp)

# Join air/water data frames for linear regression
airwater_join <- water %>% 
  left_join(air, by = "longdate")

# Create equation formula for regression equation

lm_eqn <- function(df){
    m <- lm(water_max ~ air_mean, df);
    eq <- substitute(italic(y) == a + b %.% italic(x)*","~~italic(r)^2~"="~r2, 
         list(a = format(unname(coef(m)[1]), digits = 3),
              b = format(unname(coef(m)[2]), digits = 3),
             r2 = format(summary(m)$adj.r.squared, digits = 3)))
    as.character(as.expression(eq));
}                 

# Calculate Pearson correlation coefficient
correlation <- cor(water$max_temp, air$mean_temp, method = 'pearson')
correlation

fit <- lm(airwater_join$water_max ~ airwater_join$air_mean)
summary(fit)

  
# Create regression plot
reg_plot <- ggplot(airwater_join, aes(x=air_mean, y = water_max)) +
  geom_point(color='blue') +
  geom_smooth(method = "lm", se = TRUE, colour = 'black') +
  theme_light() + 
 labs(title="Fish Cr. at mouth (Air - Copeland Cr. at mouth)",
       subtitle = "Max Water Temp ~ Mean Air Temp",
       x="Daily Mean Air temperature (°C)",
       y="Daily Max Water temperature (°C)") + 
geom_text(x = 15, y = 17.5, label = lm_eqn(airwater_join), parse = TRUE)  + 
geom_text(x = 15, y = 17, label = paste("Pearson R:", round(correlation,3))) +
  theme(plot.title = element_text(size=12, face="bold"),
        plot.subtitle = element_text(size = 11)) +
  theme(axis.text=element_text(size=11),
        axis.title=element_text(size=14))

reg_plot

#Export plot to file
#ggsave(file="2024 Logger Sites/Water/Fig Plots/regression_15110003.png", height = 4, width = 6)
 
```

#3.5C Warming Climate Scenario Predict
```{r}
# y = 7.98+0.645x	
S_AboveBigBend_climate <- airwater_join %>%
 mutate(air_pred = air_mean + 3.5) %>% 
 mutate(water_pred = 7.98 + (0.645*air_pred)) %>%
 filter(water_pred > 20) 

# y=6.32+0.605x	
S_BelowBigBend_climate <- airwater_join %>%
 mutate(air_pred = air_mean + 3.5) %>% 
 mutate(water_pred = 6.32 + (0.605*air_pred)) %>%
 filter(water_pred > 20) 

# y=7.94+0.556x	
UpperCant_climate <- airwater_join %>%
 mutate(air_pred = air_mean + 3.5) %>% 
 mutate(water_pred = 7.94 + (0.556*air_pred)) %>%
 filter(water_pred > 20) 

# y=6.71+0.48x	

NUmpqua_AboveSteam_climate <- airwater_join %>%
 mutate(air_pred = air_mean + 3.5) %>% 
 mutate(water_pred = 6.71 + (0.48*air_pred)) %>%
 filter(water_pred > 20)

# y=5.37 + 0.566	
BigBend_climate <- airwater_join %>%
 mutate(air_pred = air_mean + 3.5) %>% 
 mutate(water_pred = 5.37 + (0.566*air_pred)) %>%
 filter(water_pred > 20)

# y=7.04 + 0.727x	
SteamCant_climate <- airwater_join %>%
 mutate(air_pred = air_mean + 3.5) %>% 
 mutate(water_pred = 7.04 + (0.727*air_pred)) %>%
 filter(water_pred > 20)

# y=7.9+0.401x	
FishCr_climate <- airwater_join %>%
 mutate(air_pred = air_mean + 3.5) %>% 
 mutate(water_pred = 7.9 + (0.401*air_pred)) %>%
 filter(water_pred > 20)
```



